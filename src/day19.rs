...
//     let (rules, molecule) = parse_input(&input)?;
//     // Turn molecules into indices
//     let mut molecule_map = HashMap::new();
//     molecule_map.insert("e".to_owned(), 0);
//     fn map_molecule(molecule_map: &mut HashMap<String, u8>, s: &str) -> u8 {
//         if let Some(&key) = molecule_map.get(s) {
//             key
//         } else {
//             let key = molecule_map.len() as u8;
//             molecule_map.insert(s.to_owned(), key);
//             key
//         }
//     }
//     fn extract_molecules(molecule_map: &mut HashMap<String, u8>, data: &str) -> Vec<u8> {
...
//                 continue;
//             }
//             let s = &data[start_index..i];
//             keys.push(map_molecule(molecule_map, s));
//             start_index = i;
//         }
//         let s = &data[start_index..];
//         keys.push(map_molecule(molecule_map, s));
//         keys
//     }
//     for (from, _) in &rules {
//         map_molecule(&mut molecule_map, &from);
//     }
//     let mut transformed = Vec::with_capacity(molecule_map.len());
//     transformed.resize(molecule_map.len(), Vec::default());
//     for (from, to) in rules {
//         let key = map_molecule(&mut molecule_map, &from);
//         let values = extract_molecules(&mut molecule_map, &to);
//         transformed[key as usize].push(values);
//     }
//     let rules = transformed;
//     let molecule = extract_molecules(&mut molecule_map, &molecule);
...
//         step_count: usize,
//     ) -> Option<usize> {
//         let mut working_copy = current_state.clone();
//         for (i, &entry) in current_state.iter().enumerate() {
//             if entry as usize >= rules.len() {
//                 continue;
//             }
//             for replacement in &rules[entry as usize] {
//                 working_copy.splice(i..i + 1, replacement.iter().cloned());
//                 match working_copy.len().cmp(&molecule.len()) {
//                     Ordering::Greater => {}
//                     Ordering::Less => {
//                         if let Some(count) = visit(molecule, rules, &working_copy, step_count + 1) {
//                             return Some(count);
//                         }
//                     }
//                     Ordering::Equal => {
//                         if &working_copy == molecule {
//                             return Some(step_count + 1);
//                         }
//                     }
//                 }
//                 working_copy.splice(i..i + replacement.len(), std::iter::once(current_state[i]));
//                 debug_assert_eq!(&working_copy, current_state);
//             }
//         }
...
//     visit(&molecule, &rules, &vec![0], 0).ok_or(Error::Input("cannot solve for input"))
// }
